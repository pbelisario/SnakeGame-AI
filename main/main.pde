...
// Set true if you would like to play
...
Button loadButton;
Button saveButton;
Button increaseMutation;
Button decreaseMutation;
...
void setup()
{
  font = createFont("FreeMono.ttf", 32);
  ...
  increaseMutation = new Button(340,85,20,20,"+");
  decreaseMutation = new Button(365,85,20,20,"-");
  frameRate(fps);
...
void draw() 
{
  ...
  if(isHuman) {
    ...
    if(!modelLoaded) 
    {
      if(pop.done()) 
      {
          println("OK" + pop.bestSnake.score);
      ...
      textSize(20);
      textAlign(LEFT);
      text("GEN : "+pop.generation,120,70);
      text("BEST FITNESS : "+pop.bestFitness,120,50);
      text("MOVES LEFT : "+pop.bestSnake.numberOfMoves,120,90);
      ...
      increaseMutation.show();
      decreaseMutation.show();
    } else 
     {
        model.look();
        model.myDecisionIs();
        model.move();
        model.show();
        model.brainISH.show(0,0,360,790,model.snakeVision, model.snakeDecision);
        if(model.isDead) {
          Snake newModel = new Snake();
          newModel.brainISH = model.brainISH.clone();
          model = newModel;
...
void fileSelectedIn(File selection) {
  ...
  if (selection == null) {
    println("It's Done, Try to re-open it.");
  } else {
...
void fileSelectedOut(File selection) {
  if (selection == null) {
    ...
    Matrix[] modelWeights = modelToSave.brainISH.pull();
...
void mousePressed() {
   ...
   if(increaseMutation.collide(mouseX,mouseY)) {
      mutationRate *= 2;
      defaultmutation = mutationRate;
   }
   if(decreaseMutation.collide(mouseX,mouseY)) {
      mutationRate /= 2;
      defaultmutation = mutationRate;
   }
}
